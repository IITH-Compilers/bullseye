...
struct check_poly { isl::pw_qpolynomial _polynomial; isl::basic_set input_domain; long cache_size;} ;

std::vector<isl::qpolynomial> BullsEye::_combinations_(isl_pw_qpolynomial* poly, int N, int K, std::vector<isl::qpolynomial> mon) {
    
    std::vector<isl::qpolynomial> temp(mon);
    std::string bitmask(K, 1);  
    bitmask.resize(N, 0);  
    isl::space sp = isl::manage(isl_space_copy(isl_pw_qpolynomial_get_domain_space(poly))); 
    ...
    do {
        ...
        for (int i = 0; i < N; ++i)
        {
            if (bitmask[i]) {
                ...
                qtemp2 = isl::manage(isl_qpolynomial_mul(qtemp2.release(),qtemp3.release()));
            }
        }
    ...
    for(auto i : mon){
        ...
        qtemp3 = isl::manage(isl_qpolynomial_mul(qtemp4.release(),qtemp3.release()));
...
struct oct BullsEye::findLPIntervalBounds(int index1, int index2, int boundtype1, int boundtype2, int varsize, int psize, std::vector<std::vector<long> > &transpose, long limit,std::vector<std::vector<long> > &hmatrix){
...
struct oct BullsEye::findLPBounds(int index1, int index2, int boundtype1, int boundtype2, int varsize, int psize, std::vector<std::vector<long> > &transpose, long limit,std::vector<std::vector<long> > &hmatrix){
    struct oct constraint;
    constraint.constant = 0;
    IloEnv env;
    IloModel model(env);
...
struct oct BullsEye::findLPBoundsNeg(int index1, int index2, int boundtype1, int boundtype2, int varsize, int psize, std::vector<std::vector<long> > &transpose, long limit,std::vector<std::vector<long> > &hmatrix){
    struct oct constraint;
    constraint.constant = 0;
    IloEnv env;
    IloModel model(env);
...
isl::pw_qpolynomial BullsEye::clean_poly_lcm(isl::qpolynomial qcopy){ 
    ...
    isl::space sp = isl::manage(isl_space_copy(isl_qpolynomial_get_domain_space(qcopy.get()))); 
    isl::pw_qpolynomial pwq1 = isl::manage(isl_pw_qpolynomial_from_qpolynomial(isl_qpolynomial_zero_on_domain(sp.get()))); 
    isl::local_space ls = isl::manage(isl_local_space_from_space(sp.release())); 
    ...
    auto findMultipliers = [&](isl::term Term) {
      ...
      isl::pw_qpolynomial k_pw = isl::manage(isl_pw_qpolynomial_from_qpolynomial(k_new.release()));
      ...
      isl::pw_qpolynomial pwq2 = isl::manage(isl_pw_qpolynomial_read_from_str(qcopy.get_ctx().get(),s_poly.c_str())); 
    ...
    isl::qpolynomial qtemp = isl::manage(isl_qpolynomial_from_aff(a1.release()));
    pwq1 = pwq1.mul(isl::manage(isl_pw_qpolynomial_from_qpolynomial(qtemp.release())));
    qcopy.release(); 
    return pwq1;
 }
...
isl_stat BullsEye::vertex_count(__isl_take isl_vertex *vertex, void *user) {
  ...
  isl_multi_val* mval =  isl_multi_aff_get_constant_multi_val(isl_vertex_get_expr(vertex));
  ...
  struct oct b1 = findLPBounds(index1,index2,Values[index1],Values[index2],dims,psize,transpose,512,hmatrix);
  struct oct b2 = findLPBoundsNeg(index1,index2,Values[index1],Values[index2],dims,psize,transpose,512,hmatrix);
  
  if(b1.coeff[index1+1] != 0 && b1.coeff[index2+1] != 0) {
 
    isl::constraint lowerConsx = isl::constraint::alloc_inequality(lspace);
    lowerConsx = lowerConsx.set_coefficient_si(isl::dim::set,index1,b1.coeff[index1+1]);
    lowerConsx = lowerConsx.set_coefficient_si(isl::dim::set,index2,b1.coeff[index2+1]);
    isl::val v = isl::manage(isl_val_int_from_si(lowerConsx.get_ctx().get(),b1.constant));
  ...
  if(b2.coeff[index1+1] != 0 && b2.coeff[index2+1] != 0) { 
    ...
    isl::val v = isl::manage(isl_val_int_from_si(sec.get_ctx().get(),b2.constant));
...
isl_stat BullsEye::vertex_count_interval(__isl_take isl_vertex *vertex, void *user) {
  ...
  struct oct b1 = findLPIntervalBounds(index1,index2,Values[index1],Values[index2],dims,psize,transpose,512,hmatrix); 
...
isl_stat BullsEye::vertex_test(__isl_take isl_vertex *vertex, void *user) {
  ...
  for (int i = 0; i < Point.get_space().dim(isl::dim::set); ++i) {
    auto Value = mv.get_val(i);  
    if(!Value.is_int()) Value = Value.floor();
    Point = Point.set_coordinate_val(isl::dim::set,i,Value);
    Value.release();  
  }  

  isl::val StackDistance = isl::manage(isl_pw_qpolynomial_eval(poly.release(),Point.release()));
...
// Count Approximate cache misses using Handelman-Octagon counting of stack distance polynomials
std::vector<long> BullsEye::calculateApproximateCapacityMisses(piece Piece, std::vector<int> NonAffine, std::vector<int> Affine, std::vector<long> cache_size) {      
     
    isl::pw_qpolynomial _polynomial = isl::manage(isl_pw_qpolynomial_from_qpolynomial(isl_qpolynomial_copy(Piece.Polynomial.get())));;
    ...
    trial_set_new = isl_set_project_out(trial_set_new, isl_dim_set, isl_set_n_dim(trial_set_new)-1 , 1);
    isl_set* trial_set = isl_set_remove_divs(isl_set_copy(trial_set_new));
    _polynomial = _polynomial.drop_dims(isl::dim::in ,isl_set_n_dim(trial_set_new) , 1); 
    ...
    for(auto cache_limit : cache_size) {
      ...
      if (isl_set_n_dim(trial_set) > 3) { 
            auto countCacheMisses = [&](isl::basic_set All) { 
                ...
                auto countSubdomain = [&](isl::point Point) {
                    ...
                    if(iterCount % enum_length == 0){ 
                        ...
                        auto Expression = BullsEye::extractAffineExpression(Piece.Polynomial, Piece.Domain, Values); 
                        if (Expression.is_cst()) { 
                            long Size = isl::cardinality(Domain);
                            auto Constant = Expression.get_constant_val();  
                            auto Misses = Constant.gt(isl::val(Constant.get_ctx(), cache_limit)) ? Size : 0; 
                            cache_misses += Misses * enum_length; 
                        } else {  
                            input_domain = Domain;
                            expression = Expression;  
                            auto Misses = BullsEye::countAffineDimensions(Piece, cache_size);
                            std::transform(Misses.begin(), Misses.end(), Misses.begin(), [&enum_length](auto& c){return c * enum_length;});
        ...
        isl::space sp = isl::manage(isl_space_copy(isl_pw_qpolynomial_get_domain_space(qcopy.copy()))); 
        isl::local_space ls = isl::manage(isl_local_space_from_space(sp.release())); 
        isl::aff affine_limit = isl::manage(isl_aff_zero_on_domain(ls.release()));
        affine_limit = affine_limit.set_constant_si(cache_limit);   
        isl::qpolynomial qtemp = isl::manage(isl_qpolynomial_from_aff(affine_limit.release()));
        isl::pw_qpolynomial pwq1 = isl::manage(isl_pw_qpolynomial_from_qpolynomial(qtemp.release()));  
        ...
        for(auto &Domain_c : domains) {
            ...
            if(!interval_bounds){  
                ...
                if(std::adjacent_find( count_poly.begin(), count_poly.end(), std::not_equal_to<>() ) == count_poly.end()) { 
                    if(count_poly[0] == false){
                        totalMissCount = 0;
                        Domain_.release();
                        continue; 
                    } else { 
                        isl::val res = isl::manage(isl_set_card(isl::set(Domain_c).release())).max();
            ...
            auto getMonomial = [&](isl::constraint cons){ 
                ...
                isl::qpolynomial q = isl::manage(isl_qpolynomial_from_aff(taf.release())); 
                mono.push_back(q);
                return isl::stat::ok();
            };

            Domain_.foreach_constraint(getMonomial); 
            std::vector<isl::qpolynomial> products = BullsEye::_combinations_(polyModified.release(),mono.size(),2,mono); 
            ...
            std::vector<std::vector<long> > hmatrix(combs,std::vector<long>(products_size,0)); 
            int col = 0;
            std::string bmask = ""; 
            for(auto k : products) { 
                ...
                auto analyzeTerms = [&](isl::term Term) {  
                    ...
                    for (int i = 0; i < Term.dim(isl::dim::set); i++) { 
                        ...
                        if(val>1){
                            q = isl::manage(isl_qpolynomial_from_term(isl_term_copy(Term.get()))); 
                            k_copy = isl::manage(isl_qpolynomial_sub(k_copy.release(),q.release()));
                        }
                    } 
            ...
            std::vector<std::vector<long > > transpose(products_size,std::vector<long >(combs,0));
            ...
            isl::basic_set working_set = isl::manage(isl_basic_set_copy(Domain_c.release()));  
            try { 
                ...
                if(!interval_bounds && (isl_set_n_dim(trial_set) == 2)){ 
                    struct data_poly data = {0,1,dims,products_size,transpose,limit,hmatrix,ls};
                    isl_vertices* verts = isl_basic_set_compute_vertices(Domain_.release());  
                    isl_vertices_foreach_vertex(verts, vertex_count, &data);
                    isl_vertices_free(verts); 
                    for(auto con: constraint_list){
                        init_bset = isl::manage(isl_basic_set_add_constraint(init_bset.release(),con.release()));
                    } 
                } 
                if(interval_bounds && (isl_set_n_dim(trial_set) == 2)){ 
                    struct data_poly data = {0,1,dims,products_size,transpose,limit,hmatrix,ls};
                    ...
                    isl_vertices_free(verts); 
                    isl_vertices* verts2 = isl_basic_set_compute_vertices(Domain_.release()); 
                    struct data_poly data2 = {1,0,dims,products_size,transpose,limit,hmatrix,ls}; 
                ...
                if(!interval_bounds && (isl_set_n_dim(trial_set) == 3)){
                    ...
                    isl_vertices_foreach_vertex(verts, vertex_count, &data);
                    isl_vertices_free(verts); 
                    isl_vertices* verts2 = isl_basic_set_compute_vertices(Domain_.get()); 
                    struct data_poly data2 = {1,2,dims,products_size,transpose,limit,hmatrix,ls}; 
                    isl_vertices_foreach_vertex(verts2, vertex_count, &data2);
                    isl_vertices_free(verts2); 
                    isl_vertices* verts3 = isl_basic_set_compute_vertices(Domain_.release()); 
                    struct data_poly data3 = {2,0,dims,products_size,transpose,limit,hmatrix,ls}; 
                ...
                working_set = isl::manage(isl_basic_set_intersect(working_set.release(),init_bset.release())); 
                isl::val res = isl::manage(isl_set_card(isl::set(working_set).release())).max(); 
                totalMissCount = (long long) (res.get_num_si()); 
                cache_misses += totalMissCount;
                res.release();
                sp.release();
                ...
                std::cerr << "Error in counting approximate cache misses: " << e << std::endl;
                e.end();
            }
...
isl::aff BullsEye::extractAffineExpression(isl::qpolynomial Polynomial, isl::set Domain, std::map<int, long> Values) { 
...
void BullsEye::setcolumn( std::string bmask, int column, long long coeff, std::vector<std::vector<long > > &hm ){
...
std::vector<long> BullsEye::countAffineDimensions(piece Piece, std::vector<long> Limits) { 
