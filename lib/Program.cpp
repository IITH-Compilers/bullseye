...
void Program::extractScop(std::string SourceFile, std::string ScopFunction) {
  ...
  pet_scop *PetScop = pet_scop_extract_from_C_source(
      Context_.get(), SourceFile.c_str(), Function);
  if (PetScop == nullptr) {
  ...
  // (the tagged accesses allow us to distinguish multiple accesses of the same
  // array)
  ...
  ScopLoc_ = std::make_pair(pet_loc_get_start(PetScop->loc),
                            pet_loc_get_end(PetScop->loc));
      ...
      if (pet_expr_access_is_read(Expr) || pet_expr_access_is_write(Expr)) {
        ...
        if (Index.dim(isl::dim::out) > 0 && Index.has_tuple_id(isl::dim::out)) {
          ...
          do {
            ...
            Iter = std::find_if(Iter, AccessInfos->end(),
...
void Program::extendSchedule() {
  ...
  auto extendSchedule = [&](isl::map Schedule) {
    ...
    if (!(ReadReferences_[Statement].empty() &&
          WriteReferences_[Statement].empty())) {
      // extend the schedule
      Schedule =
          Schedule.insert_dims(isl::dim::in, Schedule.dim(isl::dim::in), 1);
      Schedule =
          Schedule.insert_dims(isl::dim::out, Schedule.dim(isl::dim::out), 1);
      ...
      isl::pw_aff VarIn = isl::pw_aff::var_on_domain(
          LSIn, isl::dim::set, Schedule.dim(isl::dim::in) - 1);
      isl::pw_aff VarOut = isl::pw_aff::var_on_domain(
          LSOut, isl::dim::set, Schedule.dim(isl::dim::out) - 1);
      ...
      isl::pw_aff Max = VarIn * 0 + ReadReferences_[Statement].size() +
                        WriteReferences_[Statement].size();
  ...
  auto extractReads = [&](isl::map Map) {
    if (ElementSizes_.count(Map.get_tuple_name(isl::dim::out)) > 0) {
      std::string Statement =
          Map.domain().unwrap().get_tuple_name(isl::dim::in);
  ...
  for (auto &ReadReferences : ReadReferences_) {
    for (int i = 0; i < ReadReferences.second.size(); ++i) {
      AccessInfos_[ReadReferences.first].push_back(
          {ReadReferences.first + "(R" + std::to_string(i) + ")",
           ReadReferences.second[i], Read, 0, 0, 0});
    }
  }
  for (auto &WriteReferences : WriteReferences_) {
    for (int i = 0; i < WriteReferences.second.size(); ++i) {
      AccessInfos_[WriteReferences.first].push_back(
          {WriteReferences.first + "(W" + std::to_string(i) + ")",
           WriteReferences.second[i], Write, 0, 0, 0});
    }
  }
}
...
isl::union_map Program::extendAccesses(isl::union_map Accesses,
  ...
  auto extendAccesses = [&](isl::map Access) {
    std::string Statement =
        Access.domain().unwrap().get_tuple_name(isl::dim::in);
    std::string Reference =
    ...
    if (ElementSizes_.count(Array) > 0) {
      ...
      long Distance = 0;
      if (WriteReferences) {
        auto Iter = std::find(WriteReferences_[Statement].begin(),
                              WriteReferences_[Statement].end(), Reference);
        assert(Iter != WriteReferences_[Statement].end());
        Distance = std::distance(WriteReferences_[Statement].begin(), Iter) +
                   ReadReferences_[Statement].size();
      } else {
        auto Iter = std::find(ReadReferences_[Statement].begin(),
                              ReadReferences_[Statement].end(), Reference);
      ...
      isl::pw_aff VarIn = isl::pw_aff::var_on_domain(
          LSIn, isl::dim::set, Access.dim(isl::dim::in) - 1);
...
void Program::computeAccessToLine(isl::set Parameters) {
  if (AccessToLine_.is_null()) {
    ...
    for (auto Array : ArrayExtents_) {
      ...
      isl::map AccessToArray =
          isl::map::identity(Extent.get_space().map_from_set());
      AccessToElement_ = AccessToElement_.unite(isl::union_map(AccessToArray));
      // compute elements per cache line
      long ElementsPerCacheLine =
          MachineModel_.CacheLineSize / ElementSizes_[Name];
      AccessToArray =
          introduceCacheLines(Name, AccessToArray, ElementsPerCacheLine);
    ...
    // AccessToElement_ = AccessToElement_.remove_redundancies();
    AccessToElement_ = AccessToElement_.coalesce();
    // AccessToElement_.dump();
  }
}

isl::map Program::introduceCacheLines(std::string Name, isl::map AccessToArray,
                                      long ElementsPerCacheLine) const {
  // introduce additional dimension that divides the innermost dimension by the
  // cache line size
  ...
  isl::set ConstraintTwo =
      Var.le_set(ElementsPerCacheLine * VarPrime + (ElementsPerCacheLine - 1));
  AccessToArray = AccessToArray.intersect_range(ConstraintOne)
                      .intersect_range(ConstraintTwo);
...
isl::map Program::introduceCacheSets(std::string Name, isl::map AccessToArray,
