...
void emulateCopy(int N1, int N2, int N3, int CacheLineSize,
  ...
  auto CLA = [&](int i, int j, int k) {
    return i * StrideA * N2 + j * StrideA + k / CacheLineSize;
  };
  auto CLB = [&](int i, int j, int k) {
    return OffsetB + i * StrideB * N2 + j * StrideB + k / CacheLineSize;
  };
  auto CLT = [&](int i, int j, int k) {
    return OffsetT + i * StrideT * N2 + j * StrideT + k / CacheLineSize;
  };
  // run copy
  for (int i = 0; i < N1; i++)
    for (int j = 0; j < N2; j++)
...
TEST_F(CopyTest, CapacityMissEven) {
  ...
  int N2 = 16;
  int N3 = 8;
  int CacheLines = 2 * N1 * N2 * ((N3 + CacheLineSize - 1) / CacheLineSize) +
                   N2 * N3 * ((N1 + CacheLineSize - 1) / CacheLineSize);
...
TEST_F(CopyTest, CompulsoryMissEven) {
  ...
  int N3 = 8;
  int CacheLines = 2 * N1 * N2 * ((N3 + CacheLineSize - 1) / CacheLineSize) +
                   N3 * N2 * ((N1 + CacheLineSize - 1) / CacheLineSize);
  std::vector<NamedLong> Parameters = {std::make_pair(std::string("N1"), N1),
                                       std::make_pair(std::string("N2"), N2),
                                       std::make_pair(std::string("N3"), N3)};
  // emulate the stack distances
