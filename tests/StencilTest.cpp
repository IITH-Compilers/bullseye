...
void emulateStencil(int N1, int N2, int CacheLineSize,
  ...
  auto CLB = [&](int i, int j) {
    return Offset + i * Stride + j / CacheLineSize;
  };
  // run stencil
  for (int t = 0; t < N1; t++) {
    for (int i = 1; i < N2 - 1; i++)
      for (int j = 1; j < N2 - 1; j++)
        ...
        Emulator.accessMemory("S0", TimeStamp,
                              {CLA(i, j), CLA(i, j - 1), CLA(i, j + 1),
                               CLA(i + 1, j), CLA(i - 1, j), CLB(i, j)});
    for (int i = 1; i < N2 - 1; i++)
      for (int j = 1; j < N2 - 1; j++)
        // A[i][j] = 0.2 * (B[i][j] + B[i][j - 1] + B[i][1 + j] + B[1 + i][j] +
        // B[i - 1][j]);
        Emulator.accessMemory("S1", TimeStamp,
                              {CLB(i, j), CLB(i, j - 1), CLB(i, j + 1),
                               CLB(i + 1, j), CLB(i - 1, j), CLA(i, j)});
  }
}
